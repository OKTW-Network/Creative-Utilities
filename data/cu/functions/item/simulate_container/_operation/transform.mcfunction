...
execute store result score #1 calcu_temp run data get storage cu:item simulate_container._current.value.shift
execute if score #1 calcu_temp matches ..-1 run scoreboard players operation #1 calcu_temp *= #-1 num
execute store result storage cu:item simulate_container._insert[{_this:1b}].shift int 1.0 run scoreboard players get #1 calcu_temp
...
execute if data storage cu:item simulate_container._current.value{method:"prepend"} run scoreboard players set #1 calcu_temp 1
execute unless data storage cu:item simulate_container._insert[1] run scoreboard players set #1 calcu_temp 1
execute if score #1 calcu_temp matches 1 run function cu:item/simulate_container/_reset_insert_status
