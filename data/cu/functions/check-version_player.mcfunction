function cu:version

scoreboard players operation #new_x.*.*-* Version = #cu_currently_version_x.*.*-* Meta
scoreboard players operation #new_*.x.*-* Version = #cu_currently_version_*.x.*-* Meta
scoreboard players operation #new_*.*.x-* Version = #cu_currently_version_*.*.x-* Meta
scoreboard players operation #new_*.*.*-x Version = #cu_currently_version_*.*.*-x Meta
scoreboard players operation #currently_x.*.*-* Version = @s VersionCUX
scoreboard players operation #currently_*.x.*-* Version = @s VersionCUY
scoreboard players operation #currently_*.*.x-* Version = @s VersionCUZ
scoreboard players operation #currently_*.*.*-x Version = @s VersionCUA
function cu:version/check

execute if score #change Version matches 1 run function cu:debug/remove-init_tag

scoreboard players operation @s VersionCUX = #cu_currently_version_x.*.*-* Meta
scoreboard players operation @s VersionCUY = #cu_currently_version_*.x.*-* Meta
scoreboard players operation @s VersionCUZ = #cu_currently_version_*.*.x-* Meta
scoreboard players operation @s VersionCUA = #cu_currently_version_*.*.*-x Meta
