...
tellraw @a[tag=!self,tag=cu._tag.listenDebugMessage] [{"selector":"@s"}," >>> Deleted ",{"score":{"name":"#dataStorage.result","objective":"cu"}}," data container",{"nbt":"plural.result","storage":"cu:string","interpret":false}," that is match ",{"nbt":"input","storage":"cu:data_storage","interpret":false}]
