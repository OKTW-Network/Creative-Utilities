...
tellraw @a[tag=!self,tag=cu._tag.debugView] [{"selector":"@s"}," >>> Deleted ",{"score":{"name":"#result","objective":"cu.dataStorage"}}," data container",{"nbt":"plural.result","storage":"cu:string","interpret":false}," that is match ",{"nbt":"input","storage":"cu:data_storage","interpret":false}]
