scoreboard players set #value.string.broadcast_raw.FUNCTION_STAGE cu-io 0
# Result
#  none : chunk not loaded or error
#  0..1 : successful
scoreboard players reset #value.string.broadcast_raw.Result cu-io
execute in cu:simulation unless loaded 0 9 0 run return run function cu:value/string/broadcast_raw/_return_fail

scoreboard players set #value.string.broadcast_raw.FUNCTION_STAGE cu-io 1
...
execute store success score #value.string.broadcast_raw.Result cu-io as @e[tag=cu-dummy.value.string.broadcast_raw.speaker] run say @e[tag=cu-dummy.value.string.broadcast_raw.message]
kill @e[tag=cu-dummy.value.string.broadcast_raw]

scoreboard players set #value.string.broadcast_raw.FUNCTION_STAGE cu-io -1
function cu:value/string/broadcast_raw/_reset_function
