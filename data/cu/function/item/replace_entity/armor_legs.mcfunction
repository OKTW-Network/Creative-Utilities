...
execute if score #item.replace_entity.chunk_load_condition cu-internal matches 1 store result score #item.replace_entity.armor_legs.Result cu-io if function cu:item/replace_entity/_func/setup_container_block in cu:simulation run item replace entity @s armor.legs from block 0 1 0 container.0
execute if score #item.replace_entity.chunk_load_condition cu-internal matches 2 store result score #item.replace_entity.armor_legs.Result cu-io if function cu:item/replace_entity/_func/setup_item_content_entity run item replace entity @s armor.legs from entity @e[type=minecraft:item_display,distance=0..,limit=1,tag=cu-dummy.item.replace_entity.item_content] contents
