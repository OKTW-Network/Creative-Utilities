...
execute if score #item.replace.chunk_load_condition cu-internal matches 1 store result score #item.replace.off_hand.Result cu-io if function cu:item/replace/_func/setup_container_block in cu:simulation run item replace entity @s weapon.offhand from block 0 1 0 container.0
