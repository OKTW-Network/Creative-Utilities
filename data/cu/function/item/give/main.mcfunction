...
execute unless data storage cu:io item.give.Input run return run function cu:item/give/_return_fail
# Option.omit_owner
...
data modify storage cu:io item.summon.Input set from storage cu:io item.give.Input
execute if score #item.give.Option.omit_owner cu-io matches 0 run data modify storage cu:io item.summon.Option.owner set from entity @s UUID
execute store result score #item.give.Result cu-io at @s run function cu:item/summon/main
