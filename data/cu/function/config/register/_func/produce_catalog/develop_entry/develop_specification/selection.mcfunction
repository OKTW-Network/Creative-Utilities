...
execute store result score #data.digit.sequence.Option.count cu-io run data get storage cu:internal config.register.produce_catalog.entry_in_development.specification.select_min
scoreboard players remove #data.digit.sequence.Option.count cu-io 1
function cu:data/digit/sequence/main
return run data modify storage cu:internal config.register.produce_catalog.entry_in_development.specification.default set from storage cu:io data.digit.sequence.Result
