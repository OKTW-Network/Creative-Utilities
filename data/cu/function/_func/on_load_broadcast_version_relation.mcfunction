execute store result score #1 cu-internal run data get storage cu:storage world[{id:'cu:world_config'}].data[{namespace:'cu'}].record[{id:'broadcast_version_on_load'}].value
execute if score #1 cu-internal matches 0 run return 0
...
execute if score #1 cu-internal matches 2 run scoreboard players set #version.compare.broadcast_relation.Option.suppress_identical cu-io 1
